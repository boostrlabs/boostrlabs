...
const support = [
  ["s1_support_tracklist", "Instagram", "Support · Tracklist WESTDETRO", "Like + comentario + repost/share del tracklist oficial. Las tres acciones juntas completan la quest.", content.tracklist, 100, 10],
  ["s1_comments_tracklist", "Instagram", "Comment Run · Tracklist WESTDETRO", "Deja 10 comentarios en el post oficial. Comentarios cortos y emojis cuentan: aquí se premia el push completo de 10.", content.tracklist, 250, 11],
  ["s1_support_punto_g", "Instagram", "Support · PUNTO G", "Like + comentario + repost/share del Reel oficial de PUNTO G.", content.punto_g, 100, 20],
  ["s1_comments_punto_g", "Instagram", "10 comentarios · PUNTO G", "Completa un run de 10 comentarios en este Reel. Sube evidencia donde se pueda verificar el esfuerzo.", content.punto_g, 250, 21],
  ["s1_support_caption", "Instagram", "Support · CAPTION", "Like + comentario + repost/share del Reel oficial de CAPTION.", content.caption, 100, 30],
  ["s1_comments_caption", "Instagram", "10 comentarios · CAPTION", "Completa un run de 10 comentarios en el Reel oficial de CAPTION.", content.caption, 250, 31],
  ["s1_support_sin_forzar", "Instagram", "Support · SIN FORZAR FREESTYLE", "Like + comentario + repost/share del Reel oficial.", content.sin_forzar, 100, 40],
  ["s1_comments_sin_forzar", "Instagram", "10 comentarios · SIN FORZAR", "Completa un run de 10 comentarios en el Reel oficial.", content.sin_forzar, 250, 41],
  ["s1_support_late_night", "Instagram", "Support · LATE NIGHT", "Like + comentario + repost/share del Reel oficial de LATE NIGHT.", content.late_night, 100, 50],
  ["s1_comments_late_night", "Instagram", "10 comentarios · LATE NIGHT", "Completa un run de 10 comentarios en el Reel oficial.", content.late_night, 250, 51],
  ["s1_support_es_mala", "Instagram", "Support · ES MALA PERO ASÍ ME GUSTA", "Like + comentario + repost/share del Reel oficial.", content.es_mala, 100, 60],
  ["s1_comments_es_mala", "Instagram", "10 comentarios · ES MALA...", "Completa un run de 10 comentarios en el Reel oficial.", content.es_mala, 250, 61],
  ["s1_support_baby_mama", "Instagram", "Support · BABY MAMA", "Like + comentario + repost/share del Reel oficial de BABY MAMA.", content.baby_mama, 100, 70],
  ["s1_support_tutorial", "Instagram", "Support · Cómo hacer un WESTDETRO", "Mira el tutorial y completa like + comentario + repost/share.", content.tutorial, 100, 80],
  ["s1_support_tutorial_result", "Instagram", "Support · Resultado WESTDETRO", "Apoya el resultado del tutorial con like + comentario + repost/share.", content.tutorial_result, 100, 81]
];
...
const rewards = [
  ["s1_reward_early", "WESTDETRO Early Access", "Acceso anticipado digital de Season 001.", "EARLY", 10000, null, 10],
  ["s1_reward_creator_review", "NNE Creator Review", "Feedback privado sobre una pieza de contenido o estrategia creativa.", "REVIEW", 15000, null, 20],
  ["s1_reward_shirt", "NNE / WESTDETRO T-shirt", "T-shirt físico del drop Season 001.", "TEE", 50000, null, 30],
  ["s1_reward_af1_white", "AF1 White", "Air Force 1 blancas. Talla sujeta a disponibilidad del proveedor.", "AF1", 65000, null, 40],
  ["s1_reward_af1_black", "AF1 Black", "Air Force 1 negras. Talla sujeta a disponibilidad del proveedor.", "AF1", 65000, null, 50],
  ["s1_reward_nike_tech", "Nike Tech Set", "Conjunto Nike Tech completo. Talla/color sujetos a disponibilidad.", "TECH", 150000, null, 60],
  ["s1_reward_production", "Producción Janko Diorr", "Producción musical completa con Janko Diorr, sujeta a agenda y términos del reward.", "PROD", 200000, null, 70]
];
...
export async function ensureNneSeason001(env) {
  ...
  const ready = await env.DB.prepare(`SELECT value FROM nne_runtime_flags WHERE key='season_001_seed_v2' LIMIT 1`).first();
  ...
  const statements = [
    ...
    env.DB.prepare(`UPDATE nne_quests SET status='archived', updated_at=datetime('now') WHERE status='published' AND id NOT LIKE 's1_%'`)
  ];
  ...
  const listening = [
    ["s1_listen_punto_g","PUNTO G · Know the record","s1_song_punto_g",150,90],
    ["s1_listen_caption","CAPTION · Know the record","s1_song_caption",150,91],
    ["s1_listen_late_night","LATE NIGHT · Know the record","s1_song_late_night",150,92],
    ["s1_listen_sin_forzar","SIN FORZAR · Know the record","s1_song_sin_forzar",150,93],
    ["s1_listen_es_mala","ES MALA... · Know the record","s1_song_es_mala",150,94]
  ];
  ...
  for (const [slug,title,url] of creatorSongs) {
    for (let slot=1; slot<=10; slot+=1) {
      const credits = slot <= 3 ? 500 : slot <= 6 ? 400 : 300;
      statements.push(questUpsert(env,{
        id:`s1_create_${slug}_${slot}`,
        platform:"TikTok / Reels",
        title:`CREATE · ${title} · #${slot}/10`,
        description:`Crea un TikTok o Reel ORIGINAL usando ${title}. Base: ${credits} Credits. Staff puede sumar Creativity + Performance/Impact Bonus. Cantidad tiene límite; impacto puede romperlo. Sube screenshot y agrega el link del video en la nota.\n\nReferencia: ${url}`,
        credits, sort: sort++
      }));
    }
  }
  ...
  statements.push(env.DB.prepare(`INSERT INTO nne_runtime_flags (key,value,updated_at) VALUES ('season_001_seed_v2','ready',datetime('now')) ON CONFLICT(key) DO UPDATE SET value='ready',updated_at=datetime('now')`));
