...
function safeCss(value, fallback) {
  ...
  if (text.length > 520) return fallback;
  return text;
}
...
function safeColor(value = '#feedb9') {
  const text = String(value || '').trim();
...
function normalizeItem(item = {}) {
  ...
  if (type === 'links') {
    ...
    return { type, links: links.slice(0, 24).map((row) => [String(row?.[0] || 'LINK').slice(0, 40), safeUrl(row?.[1]) || '#']) };
  }
  if (type === 'gallery') {
  ...
  if (type === 'grid') {
    ...
    return { type, tiles: tiles.slice(0, 60).map((row) => [String(row?.[0] || '').slice(0, 48), safeUrl(row?.[1]), safeUrl(row?.[2]), safeUrl(row?.[3]) || '#']).filter((row) => row[1] || row[2]) };
  }
  if (type === 'divider') {
    return {
      type,
...
export function normalizeSite(site) {
  ...
  const normalized = {
    ...
    sections: sections.slice(0, 1000).map((section, index) => {
      ...
      const label = String(section?.label || id || 'Section').slice(0, 48);
...
function itemHtml(item) {
  ...
  if (item.type === 'links') return `<div class="links">${item.links.map((row) => `<a href="${escapeHtml(row[1] || '#')}">${escapeHtml(row[0] || 'LINK')}</a>`).join('')}</div>`;
