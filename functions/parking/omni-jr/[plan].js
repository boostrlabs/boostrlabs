...
async function provisionFixedPlan(env, workspaceId) {
  const existing = await findPaymentLink(env, PLANS["8h"], workspaceId);
...
export async function onRequestGet({ env, params }) {
  ...
  if (!link?.id && key === "8h") {
    try { link = await provisionFixedPlan(env, workspaceId); } catch (error) { console.error("OMNI JR parking provision failed", error); }
  }
