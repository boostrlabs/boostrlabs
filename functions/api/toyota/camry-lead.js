...
const CAMPAIGN = "TIKTOK CAMRY 499 · TOYOTA OF HOLLYWOOD X LA CHIQUI";
const DEALER = "Toyota of Hollywood";
const SELLER = "Adriana Quintero (La Chiqui)";
const VEHICLE = "2026 Toyota Camry Hybrid";
...
export async function onRequestPost(context) {
  ...
  const lead = {
    ...
    source: clean(payload.source, 120) || "tiktok-camry-499",
  ...
  if (missing.length) {
    return jsonError("required_fields_missing", "Complete all required fields.", 400, { fields: missing });
  }
  if (!lead.phone && !lead.email) {
    return jsonError("contact_required", "Enter a phone number or email address.", 400, { fields: ["phone", "email"] });
  }
  if (lead.phone && !isValidPhone(lead.phone)) {
    return jsonError("invalid_phone", "Enter a valid phone number.", 400);
  }
  ...
  let pass;
  try {
    pass = await createToyotaPass(env, lead, { campaign: CAMPAIGN, vehicle: VEHICLE });
  } catch (error) {
    console.error(JSON.stringify({ message: "toyota_camry_pass_generation_failed", error: String(error) }));
