...
async function count(env, table, where = "") {
  try { const row = await env.DB.prepare(`SELECT COUNT(*) AS total FROM ${table} ${where}`).first(); return row?.total ?? 0; } catch { return null; }
}
async function latest(env, table, cols = "id, workspace_id, created_at", where = "") {
  try { return await env.DB.prepare(`SELECT ${cols} FROM ${table} ${where} ORDER BY created_at DESC LIMIT 1`).first(); } catch { return null; }
}
...
export async function onRequestGet({ env }) {
  ...
  if (env.DB) {
    try {
      ...
      if (db.tables.includes("audit_submissions")) {
        metrics.audits_total = await count(env, "audit_submissions");
        metrics.claimed_audits_total = await count(env, "audit_submissions", "WHERE status = 'claimed'");
        metrics.last_audit = await latest(env, "audit_submissions", "id, workspace_id, contact_email, business_name, status, created_at");
      }
      ...
      if (db.tables.includes("products")) {
        metrics.products_total = await count(env, "products", "WHERE status != 'archived'");
        metrics.active_products_total = await count(env, "products", "WHERE status = 'active'");
        metrics.last_product = await latest(env, "products", "id, workspace_id, title, product_type, status, price_amount, currency, created_at", "WHERE status != 'archived'");
      }
      if (db.tables.includes("payment_links")) {
        metrics.payment_links_total = await count(env, "payment_links", "WHERE status != 'archived'");
        metrics.active_payment_links_total = await count(env, "payment_links", "WHERE status = 'active'");
        metrics.last_payment_link = await latest(env, "payment_links", "id, workspace_id, product_id, title, status, amount_cents, currency, created_at", "WHERE status != 'archived'");
      }
      if (db.tables.includes("order_reservations")) {
        metrics.reservations_total = await count(env, "order_reservations");
        metrics.last_reservation = await latest(env, "order_reservations", "id, workspace_id, payment_link_id, product_id, status, reservation_type, created_at");
      }
      if (db.tables.includes("workspace_files")) {
        metrics.files_total = await count(env, "workspace_files", "WHERE status = 'active'");
        metrics.last_file = await latest(env, "workspace_files", "id, workspace_id, title, file_type, visibility, created_at", "WHERE status = 'active'");
      }
      if (db.tables.includes("invoices")) {
        metrics.invoices_total = await count(env, "invoices", "WHERE status != 'archived'");
        metrics.last_invoice = await latest(env, "invoices", "id, workspace_id, invoice_number, status, amount_cents, currency, created_at", "WHERE status != 'archived'");
      }
    ...
    } catch (error) { db.error = "D1 binding exists but query failed. Run migrations."; }
  }
  ...
  return json({
    ...
    products: { endpoint: "/api/products", item_endpoint: "/api/products/:id", workspace_ui: "/app/products", creates_real_workspace_products: true, stripe_required: false },
