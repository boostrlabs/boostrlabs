...
const FILENAMES = ["Johanka-3D.glb", "Johanka-3D(1).glb", "johanka-3d.glb", "johanka-doll.glb", "johanka.glb"];
...
async function resolve(bucket) {
  ...
  for (const prefix of ["public/3dmodels/", "3dmodels/", ""]) {
    try {
      ...
      const match = (page.objects || []).find((o) => String(o.key || "").toLowerCase().endsWith(".glb") && String(o.key || "").toLowerCase().includes("johanka"));
...
async function serve({ env }, head = false) {
  ...
  if (!found) return jsonError("model_not_uploaded", "Johanka GLB is not available in R2.", 404, { expected_path: "public/3dmodels/Johanka-3D.glb", accepted_filenames: FILENAMES });
  ...
  if (!object) return jsonError("model_disappeared", "Johanka GLB could not be read.", 404);
