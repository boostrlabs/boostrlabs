...
const requiredMigrations = ["0010_invite_codes.sql", "0011_seed_initial_invite_codes.sql", "0012_signup_workspace_bootstrap.sql", "0013_operational_80_foundation.sql", "0014_auth_recovery_verification.sql"];
...
const criticalUserColumns = ["username", "phone", "normalized_phone", "password_hash", "default_workspace_id", "default_persona_id", "language", "signup_source", "invite_code_id", "onboarding_status", "invite_token_hash", "invite_token_expires_at", "invite_accepted_at", "password_reset_token_hash", "password_reset_token_expires_at", "email_verification_token_hash", "email_verification_token_expires_at", "email_verified_at"];
...
export async function onRequestGet({ env }) {
  ...
  try {
    ...
    if (missingTables.length || missingColumns.length) { status = "missing_migrations"; nextSteps.push("Apply D1 migrations through 0014_auth_recovery_verification.sql."); if (missingTables.length) nextSteps.push(`Missing tables: ${missingTables.join(", ")}.`); if (missingColumns.length) nextSteps.push(`Missing columns: ${missingColumns.join(", ")}.`); }
